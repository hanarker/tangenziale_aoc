...
- Il sito ripete talvolta lo stesso tratto/svincolo in **paragrafi separati** del testo (uno per settimana di cantiere, con date diverse). Il prompt istruisce esplicitamente a unire le finestre di tutte le occorrenze invece di far vincere l'ultima. **Rischio residuo noto**: anche con questa istruzione, gpt-4o non è perfettamente deterministico (nemmeno a temperature 0) e in alcune chiamate reali ha comunque scartato silenziosamente le finestre di un'occorrenza (osservato ~2 volte su 5 tentativi ravvicinati sullo stesso testo). Non c'è validazione automatica lato codice che rilevi finestre mancanti: un cron sfortunato può salvare uno stato con una chiusura "dimenticata" finché il testo sorgente non cambia di nuovo. Se si nota un tratto/svincolo con meno finestre del previsto, la mitigazione manuale è ri-eseguire l'update (`npm run update`, che però salta l'LLM se il testo non è cambiato: bisogna forzare una nuova chiamata, es. via script ad-hoc che richiama `interpretAvvisi` direttamente).
